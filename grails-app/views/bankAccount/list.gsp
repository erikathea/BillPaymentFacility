			...
			<table>
				<thead>
					<tr>
						...
						<th><g:message code="bankAccount.branch.label" default="Branch" /></th>
				...
				<g:each in="${bankAccountInstanceList}" status="i" var="bankAccountInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${bankAccountInstance.id}">${fieldValue(bean: bankAccountInstance, field: "branch")}</g:link></td>
					
						<td>${fieldValue(bean: bankAccountInstance, field: "accountNumber")}</td>
					
						<td>${fieldValue(bean: bankAccountInstance, field: "accountBalance")}</td>
